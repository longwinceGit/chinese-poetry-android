# 诗词乐园 · 中华古诗词学习 Android App

> **品味千年诗词之美** — 一款离线可用、古风美学、寓教于乐的中华诗词学习 App

## 项目说明

纯原生 Android 古诗词学习应用，91,196 首诗词数据离线打包，无需网络。从 [chinese-poetry](https://github.com/chinese-poetry/chinese-poetry) 数据集提取整理，采用 MVVM 架构 + Navigation Component 单 Activity 多 Fragment 模式。以水墨古风为设计基调，融合拼音标注、语音朗读、趣味游戏与成长激励体系，让用户在沉浸式体验中感受中华诗词之美。
...
- **离线为王**：所有诗词数据内置 App 中，零网络依赖。飞机上、地铁里、无信号山区，随时随地畅读
- **性能过硬**：拼音转换 LRU 缓存（4096 容量）单字查询 250 倍加速；诗词 ID 索引 O(1) 查找；搜索后台线程执行 + DiffUtil 增量刷新零卡顿；分享卡 RGB_565 + OOM 防护长诗不崩溃
- **文化尊重**：释义由人工编写非机器翻译；分享卡融入印章、宣纸纹理等中国古典视觉元素；文案统一古风（"墨香氤氲，诗词正在加载……"）
